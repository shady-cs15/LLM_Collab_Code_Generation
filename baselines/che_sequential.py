...
class QwenCoopHumanEvalSequentialEvaluator:
    def __init__(self, aux_model_name, main_model_name, device="auto"):
        """
        Initialize the sequential model evaluator for CoopHumanEval.
        Main agent sees aux agent's output.
...
"""
    ...
    def main_function_formatter_sequential(
        self, prompt: str, entry_point: str, aux_function_code: str
    ) -> str:
        """
        Formatter for the main function generator in sequential mode.
        The main agent can see the auxiliary function implementation.
        """
        params = self.extract_function_params_from_prompt(prompt)
        if not params or not entry_point:
            return "Error: Could not extract function information from prompt."
...
"""
    ...
    def evaluate_sequential_completion(
        self,
        aux_completion: str,
        main_completion: str,
        test_code: str,
        ...
        """
        Evaluate a sequential model completion using the coophumaneval reward logger approach.
        """
        metrics = {
            "sample_id": sample_id,
    ...
    def evaluate_coophumaneval_sequential(
        self,
        num_samples: int = 31,
        ...
        """
        Evaluate CoopHumanEval sequential model system on the last num_samples.
        """
        ...
        print(f"Mode: SEQUENTIAL (Main agent sees Aux agent's output)")
        ...
        for sample_idx, sample in enumerate(
            ...
            for gen_idx in range(num_generations):
                try:
                    # STEP 1: Generate auxiliary function
                    ...
                    # Extract and clean the aux function
                    aux_func = extract_specific_function(
                        cleanup_code(aux_response), "aux"
                    )

                    # STEP 2: Generate main function with aux function context
                    # Main agent sees the cleaned extracted aux function
                    main_prompt = self.main_function_formatter_sequential(
                        prompt,
                        entry_point,
                        aux_func if aux_func else "# No aux function generated",
                    )
                    start_time = time.time()
                    ...
                    # Total time is sum of both agents
                    total_time = aux_time + main_time
                    ...
                    metrics = self.evaluate_sequential_completion(
                        aux_response,
                        main_response,
                        test_code,
                        entry_point,
        ...
        # Print results
        print("\n" + "=" * 60)
        print("HUMANEVAL SEQUENTIAL MODEL EVALUATION RESULTS")
        print("=" * 60)
        print(f"Aux Model: {self.aux_model_name}")
        print(f"Main Model: {self.main_model_name}")
        print(f"Generation Mode: SEQUENTIAL (Main sees Aux output)")
        print(
        ...
        if save_results:
            ...
            filename = f"coophumaneval_sequential_{int(time.time())}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="CoopHumanEval Sequential Model Evaluation"
    )
    parser.add_argument(
    ...
    evaluator = QwenCoopHumanEvalSequentialEvaluator(
    ...
    # Run evaluation
    aggregated_metrics, sample_results = evaluator.evaluate_coophumaneval_sequential(
...
if __name__ == "__main__":
    # python experiments/evaluation/che_sequential.py --generations 15 --k-values 1 3 5 10
    main()
