...
class QwenHumanEvalSequentialEvaluator:
    def __init__(self, aux_model_name, main_model_name, device="auto"):
        """
        Initialize the sequential model evaluator for HumanEval.
...
"""
    ...
    def evaluate_sequential_completion(
        ...
        """
        Evaluate a sequential model completion using the humaneval reward logger approach.
        """
        metrics = {
            "sample_id": sample_id,
    ...
    def evaluate_humaneval_sequential(
        self,
        num_samples: int = 31,
        ...
        """
        Evaluate HumanEval sequential model system on the last num_samples.
        """
        ...
        if save_results:
            ...
            filename = f"humaneval_sequential_{int(time.time())}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="HumanEval Sequential Model Evaluation"
    )
    parser.add_argument(
    ...
    evaluator = QwenHumanEvalSequentialEvaluator(
    ...
    # Run evaluation
    aggregated_metrics, sample_results = evaluator.evaluate_humaneval_sequential(
...
if __name__ == "__main__":
    # python experiments/evaluation/he_sequential.py --generations 15 --k-values 1 3 5 10
    main()
