...
def plot_combined_two_panels(plot_id=1):
    ...
    # Apply smoothing to single-turn (window_size = 8)
    single_window_size = 8
    for col in [
        "total_mean_scaled",
        "total_std_scaled",
    ...
    # Clip all scaled values to 0-100 range to prevent overflow
    for df in [aggregated_single, aggregated_mt]:
    ...
    ax2.set_title("HE | Multi-Turn", fontsize=title_size, pad=10)
    ...
    complete_filename = f"he_two_panel_{plot_id}.pdf"
    ...
    with open(complete_filename, "rb") as file:
        ...
        for page in reader.pages:
            ...
            split_point = page_width * 2 / 5  # Match writing plot's 2:3 ratio split
        ...
        left_filename = f"he_single_turn_{plot_id}.pdf"
        with open(left_filename, "wb") as left_file:
            left_writer.write(left_file)

        # Save right half (Multi-Turn with legend)
        right_filename = f"he_multi_turn_{plot_id}.pdf"
        with open(right_filename, "wb") as right_file:
            right_writer.write(right_file)

    print(f"Generated plot {plot_id}:")
    print(f"  Complete: {complete_filename}")
    print(f"  Single: {left_filename}")
    print(f"  Multi: {right_filename}")


# Run the plotting function
if __name__ == "__main__":
    # Generate single set of plots
    plot_combined_two_panels(plot_id=1)
