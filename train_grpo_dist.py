...
import torch  # [dist] Add torch import for fp16 support
...
# [dist]
# Import distributed version of GRPOTrainer
from comlrl.trainers.grpo_dist import GRPOTrainer
...
def main():
    ...
    # [dist]
    # Add _dist suffix to output directory
    output_dir = os.path.join(output_base_dir, f"job_{slurm_job_id}_dist")
    ...
    # [dist]
    # Load model in fp32 - Accelerate will handle mixed precision training
    # Note: Gradient checkpointing is disabled for DDP compatibility
    model_kwargs = model_config.model_kwargs.copy()
    model_kwargs.update(
        {
            "low_cpu_mem_usage": True,  # Reduce CPU memory usage
            "use_cache": False,  # Must be disabled for training
        }
    )
    # Remove torch_dtype to let model load in default fp32
    ...
    # [dist]
    # Gradient checkpointing is incompatible with DDP
    # If memory is an issue, consider using gradient accumulation instead
    print("Note: Gradient checkpointing disabled for DDP compatibility")
    ...
    # [dist]
    # Add gradient accumulation steps for multi-GPU efficiency
    grpo_args = MAGRPOConfig(
        ...
        gradient_accumulation_steps=grpo_config.get(
        ...
        max_grad_norm=grpo_config.get("max_grad_norm", 1.0),  # Add gradient clipping
    )
    ...
    # [dist]
    # Add dist suffix to wandb name
    wandb_name = wandb_section.get("name", f"grpo_{dataset_type}_dist")
    ...
    wandb_config = {
        ...
        "tags": wandb_section.get(
            "tags", ["grpo", dataset_type, "single-agent", "distributed"]
        ),
    }
